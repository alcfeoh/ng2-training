...
fdescribe('License Plate Store checkout page', () => {
  ...
  it('should show a disposable confirmation message upon checkout', async() => {
    await checkoutPage.enterCreditCardNumber('4234567891234567');
    await checkoutPage.enterFirstName('Alain');
    await checkoutPage.enterZipcode('12345');
    await checkoutPage.clickSubmit();
    const popupTitle = await checkoutPage.getConfirmationPopupTitle();
    console.log( 'title', popupTitle);
    expect(popupTitle).toBe('Order complete');
    await checkoutPage.closeConfirmationPopup();
  });
